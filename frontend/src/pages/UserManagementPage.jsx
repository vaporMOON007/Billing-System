...
import { Users, Plus, Edit2, KeyRound, ToggleLeft, ToggleRight, X, Check, Shield, User } from 'lucide-react';
import toast from 'react-hot-toast';
import { userAPI } from '../services/api';
import { useAuth } from '../context/AuthContext';

const ROLE_LABELS = { CA: 'CA', EMPLOYEE: 'Employee' };
const ROLE_COLORS = {
                ...
                <tr key={u.id} className={`hover:bg-gray-50 transition-colors ${!u.is_active ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${ROLE_COLORS[u.role]}`}>
                      {u.role === 'CA' ? <Shield className="w-3 h-3" /> : <User className="w-3 h-3" />}
                      {ROLE_LABELS[u.role]}
                    </span>
                  </td>
                  <td className="px-4 py-3">
